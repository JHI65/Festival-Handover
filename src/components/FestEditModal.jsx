...
function FestEditModal({ fest, onSave, onSaveAsTemplate, onClose }) {
  ...
  function addDay(stageId) {
    ...
    const newDay = { id: uid(), label: t("DÍA {n}", { n: (st?.days?.length || 0) + 1 }), artists: [] };
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}
      ...
      <div style={{ background: T.card, borderRadius: "20px 20px 0 0", padding: "24px 20px 36px", width: "100%", maxWidth: 480, maxHeight: "88dvh", overflowY: "auto" }}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div style={{ fontSize: 18, fontFamily: "'Bebas Neue',sans-serif", color: T.text, letterSpacing: "0.05em" }}>{t("EDITAR FESTIVAL")}</div>
        ...
        {stages.map(st => (
          <div key={st.id} style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 10, color: T.text4, letterSpacing: "0.1em", marginBottom: 8 }}>
              {stages.length > 1 ? `${st.name} · ${t("DÍAS")}` : t("DÍAS")}
            </div>
            ...
            <button onClick={() => addDay(st.id)} style={{ ...S.addBtn, marginTop: 8, fontSize: 12, padding: "9px" }}>{t("+ Añadir día")}</button>
          </div>
        ))}
        ...
        {!tplMode ? (
          ...
          <div style={{ marginTop: 12, padding: "14px", borderRadius: 12, border: `1.5px solid ${T.border}`, background: T.card2 }}>
            <div style={{ fontSize: 10, color: T.text4, letterSpacing: "0.1em", marginBottom: 8 }}>{t("NOMBRE DE LA PLANTILLA")}</div>
            <input
              value={tplName}
