...
function BuilderNotes({ comments, onAdd, onDel }) {
  ...
  return (
    <div style={{ marginTop: 10 }}>
      <div style={{ fontSize: 9, color: T.text4, letterSpacing: "0.1em", marginBottom: 6 }}>{t("NOTAS PREVIAS")}</div>
      {comments.map((c, i) => (
      ...
      <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
        <input value={draft} onChange={e => setDraft(e.target.value)}
          ...
          placeholder={t("Añadir nota…")} style={{ ...S.input, flex: 1 }} />
...
function Builder({ onCancel, onSave, onSaveAsTemplate }) {
  ...
  const [days, setDays] = useState([{ id: uid(), label: t("DÍA 1"), artists: [] }]);
  const [expDay, setExpDay] = useState(0);

  const addDay = () => { setDays([...days, { id: uid(), label: t("DÍA {n}", { n: days.length + 1 }), artists: [] }]); setExpDay(days.length); };
  ...
  const addComment = (di, ai, txt) => {
    if (!txt.trim()) return;
    const d = [...days]; d[di].artists[ai].comments = [...(d[di].artists[ai].comments || []), txt.trim()]; setDays(d);
  };
  ...
  return (
    <div style={{ padding: "20px 16px 40px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 18, fontFamily: "'Bebas Neue',sans-serif", color: T.text, letterSpacing: "0.05em" }}>{t("NUEVO FESTIVAL")}</div>
      </div>

      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 10, color: T.text4, letterSpacing: "0.1em", marginBottom: 6 }}>{t("NOMBRE")}</div>
        <input value={name} onChange={e => setName(e.target.value)} placeholder={t("Ej: Mad Cool 26")} style={S.input} />
      </div>
      ...
      {days.map((d, di) => (
        <div key={d.id} style={S.daySection}>
          ...
          <div style={{ fontSize: 10, color: T.text4, marginTop: 4 }}>{t("{n} artistas", { n: d.artists.length })}</div>
          ...
          {expDay === di && (
            <div style={{ marginTop: 10 }}>
              {d.artists.map((a, ai) => (
                <div key={a.id} style={S.artForm}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <span style={{ fontSize: 10, color: "#D4A843", letterSpacing: "0.1em", fontWeight: 700 }}>{t("ARTISTA {n}", { n: ai + 1 })}</span>
                    <button onClick={() => delArtist(di, ai)} style={S.iconBtn}>×</button>
                  </div>
                  <input value={a.artist} onChange={e => setAF(di, ai, "artist", e.target.value)} placeholder={t("Nombre artista")} style={{ ...S.input, marginBottom: 6 }} />
                  <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                    <input value={a.console} onChange={e => setAF(di, ai, "console", e.target.value)} placeholder={t("Consola")} style={S.input} />
                    <input value={a.signal} onChange={e => setAF(di, ai, "signal", e.target.value)} placeholder={t("Señal")} style={S.input} />
                  </div>
                  <input value={a.connection} onChange={e => setAF(di, ai, "connection", e.target.value)} placeholder={t("Conexión")} style={{ ...S.input, marginBottom: 6 }} />
                  <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, marginBottom: 6, alignItems: "center" }}>
                    <input value={a.preset} onChange={e => setAF(di, ai, "preset", e.target.value)} placeholder={t("Preset")} style={{ ...S.input, flex: 1 }} />
                    ...
                    }}>{a.presetOk ? t("✓ OK") : t("preset?")}</button>
                  </div>

                  <div style={{ marginTop: 10 }}>
                    <div style={{ fontSize: 9, color: dark ? "#93c5fd" : "#2563eb", letterSpacing: "0.1em", marginBottom: 6 }}>{t("CAMPOS EXTRA")}</div>
                    {(a.extraSlots || []).map(s => (
                      <div key={s.id} style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                        <input value={s.label} onChange={e => setES(di, ai, s.id, "label", e.target.value)} placeholder={t("Etiqueta (RF…)")} style={{ ...S.input, flex: 1 }} />
                        <input value={s.value} onChange={e => setES(di, ai, s.id, "value", e.target.value)} placeholder={t("Valor")} style={{ ...S.input, flex: 1.5 }} />
                        <button onClick={() => delES(di, ai, s.id)} style={S.iconBtn}>×</button>
                      </div>
                    ))}
                    <button onClick={() => addExtraSlot(di, ai)} style={{ ...S.addBtn, color: "#2563eb", borderColor: "#bfdbfe", background: "#eff6ff" }}>{t("+ Campo")}</button>
                  </div>
              ...
              <button onClick={() => addArtist(di)} style={S.addBtn}>{t("+ Añadir artista")}</button>
            </div>
          )}
        </div>
      ))}

      <button onClick={addDay} style={{ ...S.addBtn, marginTop: 12 }}>{t("+ Añadir día")}</button>
      <button onClick={() => valid && onSave({ id: uid(), name: name.trim(), stages: [{ id: uid(), name: t("ESCENARIO PRINCIPAL"), days }] })} disabled={!valid}
        style={{ ...S.bigBtn, marginTop: 24, opacity: valid ? 1 : 0.4 }}>
        {t("GUARDAR FESTIVAL")}
      </button>
      {onSaveAsTemplate && (
        <button onClick={() => valid && onSaveAsTemplate({ id: uid(), name: name.trim(), stages: [{ id: uid(), name: t("ESCENARIO PRINCIPAL"), days }] })} disabled={!valid}
